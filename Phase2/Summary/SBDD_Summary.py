...
def sbdd_ProviderReport ():
    ...
    for row in arcpy.SearchCursor(theFGDB + "/Prov_FRQ"):
        ...
        theDBA = str(row.getValue("DBANAME")).replace(","," ")
        theProv = str(row.getValue("PROVNAME")).replace(",", " ")
...
try:
    ...
    #set up output file
    outFile = theOF + "/" + theST + "_" + str(theYear) + "_" + str(theMonth) + "_" + str(theDay) + ".csv"
    ...
    myFile.write("\n")
    myFile.write(",,Date:,," + str(theMonth) + "/" + str(theDay) + "/" + str(theYear) + "\n")   
    myFile.write("\n")
    myFile.close()
    ...
    sbdd_WriteLine("1")
    sbdd_SpeedDetail(theFD + "/BB_Service_CAInstitutions", "MAXADDOWN")
    sbdd_WriteLine("1")
    sbdd_SpeedDetail(theFD + "/BB_Service_CAInstitutions", "MAXADUP")
    sbdd_WriteLine("15")
    myFile.close()
